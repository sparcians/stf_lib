...
namespace stf {
    ...
    using RecordDeleter = PoolDeleter<STFRecord, PoolType>;

    using STFRecordPool = STFPool<STFRecord,
                                  descriptors::internal::Descriptor,
                                  descriptors::internal::NUM_DESCRIPTORS>;
};
...
//            static inline typename std::enable_if<sizeof...(Args) == 1 && type_utils::are_same<STFIFstream, std::remove_reference_t<Args>...>::value, ReturnType>::type
//            construct_(Args&&... args) {
//                auto ptr = STFRecordCache<STFRecord>::get().pop(TypeAwareSTFRecord<RecordType>::getTypeId());
